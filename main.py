...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY')  # ADD CONFIGURATION
...
class BlogPost(db.Model):
    ...
    # ~ CHILD RELATIONSHIPS ~ #
    #
    # 1. Link to  ~USER~  class
    # Create Foreign Key, 'user.id'. The 'user' refers to the tablename of User.
    author_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    # Create reference to User object. The 'posts' refers to the 'posts' property in the User class.
    author = relationship('User', back_populates='posts')

    # ~ PARENT RELATIONSHIPS ~ #
    #
    # 1. Link to  ~COMMENT~  class
    # Create reference to Comment object. The 'parent_post' refers to the 'parent_post' property in the Comment class.
    comments = relationship('Comment', back_populates='parent_post')


# CREATE COMMENT SECTION (CHILD)
class Comment(db.Model):
    ...
    # 1. Link to  ~USER~  class
    author_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    comment_author = relationship('User', back_populates='comments')  # 'comments' variable in User, not table name
    #
    # 2. Link to  ~BLOGPOST~  class
    post_id = db.Column(db.Integer, db.ForeignKey('blog_posts.id'))
    parent_post = relationship('BlogPost', back_populates='comments')  # 'comments' variable in BlogPost, not table name
...
# ONLY ADMINS CAN ENTER PAGES WITH THIS WRAPPER
...
def register():
    ...
    if form.validate_on_submit():
        if User.query.filter_by(email=request.form['email']).first() is None:
            new_user = User(
                # ~ *BUG*  IGNORE UNEXPECTED ARGUMENT FLAGS (STILL RUNS)  *BUG* ~ #
                name=form.name.data,
